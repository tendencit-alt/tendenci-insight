...
-- Auto-create delivery_order when production_order is completed
...
BEGIN
  IF NEW.status = 'concluida' AND (OLD.status IS NULL OR OLD.status <> 'concluida') THEN
    -- Resolve order_id from order_item
    ...
    -- Idempotent: skip if delivery already exists for this production_order
    SELECT id INTO v_existing
    FROM public.delivery_orders
    WHERE production_order_id = NEW.id AND tenant_id = v_tenant
    LIMIT 1;
    ...
    INSERT INTO public.delivery_orders
      (tenant_id, order_id, production_order_id, status, observacoes)
    VALUES
      (v_tenant, v_order_id, NEW.id, 'pendente',
    ...
    INSERT INTO public.cross_module_events
      (tenant_id, event_type, source_module, target_module,
       ...
       payload, status)
    VALUES
      (v_tenant, 'op_pronta_para_entrega', 'producao', 'fulfillment',
       'production_orders', NEW.id, 'orders', v_order_id,
       jsonb_build_object('production_order_id', NEW.id, 'order_id', v_order_id),
       'pending');

    -- Notify order seller / created_by
    SELECT seller_id, order_number INTO v_seller, v_order_number
    ...
    IF v_seller IS NOT NULL THEN
      ...
      VALUES
        (v_tenant, v_seller, 'fulfillment', 'op_ready',
...
DROP TRIGGER IF EXISTS trg_auto_create_delivery ON public.production_orders;
CREATE TRIGGER trg_auto_create_delivery
AFTER UPDATE ON public.production_orders
FOR EACH ROW
EXECUTE FUNCTION public.auto_create_delivery_on_production_completed();
