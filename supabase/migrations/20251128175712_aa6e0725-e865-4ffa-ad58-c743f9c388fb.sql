...
-- Primeiro, garantir que o enum user_role contém 'projetista'
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type t 
                 JOIN pg_enum e ON t.oid = e.enumtypid  
                 WHERE t.typname = 'user_role' AND e.enumlabel = 'projetista') THEN
    ALTER TYPE user_role ADD VALUE 'projetista';
  END IF;
END $$;
