...
BEGIN
  IF NEW.status = 'concluido' AND (OLD.status IS NULL OR OLD.status <> 'concluido') THEN
    ...
    SELECT seller_id, order_number INTO v_seller, v_order_number FROM public.orders WHERE id = v_order_id;
...
BEGIN
  IF NEW.status = 'concluido' AND (OLD.status IS NULL OR OLD.status != 'concluido') THEN
    UPDATE public.operational_projects
      ...
      WHERE order_id = (SELECT oi.order_id FROM public.order_items oi WHERE oi.id = NEW.order_item_id);
    PERFORM register_cross_module_event(
      'producao_concluida', 'operacional', 'comercial', 'production_orders', NEW.id, 'orders',
  ...
  RETURN NEW;
END $$;
REVOKE EXECUTE ON FUNCTION public.on_production_order_completed() FROM PUBLIC, anon;
