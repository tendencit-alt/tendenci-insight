-- Criar função para buscar estatísticas de meta do vendedor
  ...
  SELECT json_build_object(
    'meta_ativa', (
      SELECT json_build_object(
        ...
        'quantidade_meta', sg.quantidade_meta,
    ...
    'insignias', (
      SELECT COALESCE(json_agg(badge_order.badge_data ORDER BY badge_order.earned_at DESC), '[]'::json)
      FROM (
        SELECT json_build_object(
          'type', badge_type,
          'earned_at', earned_at,
