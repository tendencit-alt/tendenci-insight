...
-- Create Rustico and Nautico production types
INSERT INTO production_types (name, slug, active) VALUES
  ('Rustico', 'rustico', true),
  ('Nautico', 'nautico', true);

-- Create phase templates for Rustico
INSERT INTO production_phase_templates (production_type_id, name, slug, position, is_start_phase, is_end_phase)
SELECT pt.id, phase.name, phase.slug, phase.pos, phase.is_start, phase.is_end
FROM production_types pt
CROSS JOIN (VALUES 
  ('Aguardando', 'aguardando', 1, true, false),
  ('Compras', 'compras', 2, false, false),
  ('Inicio Produção', 'inicio-producao', 3, false, false),
  ('Finalizado', 'finalizado', 4, false, false),
  ('Entregue', 'entregue', 5, false, true)
) AS phase(name, slug, pos, is_start, is_end)
WHERE pt.name = 'Rustico';

-- Create phase templates for Nautico
INSERT INTO production_phase_templates (production_type_id, name, slug, position, is_start_phase, is_end_phase)
SELECT pt.id, phase.name, phase.slug, phase.pos, phase.is_start, phase.is_end
FROM production_types pt
CROSS JOIN (VALUES 
...
WHERE pt.name = 'Nautico';

-- Retroactively create OPs for remaining orders without OPs
DO $$
DECLARE
  v_order RECORD;
  v_item RECORD;
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Phases are auto-created by create_phases_on_op_insert trigger
      -- Just set first phase as current and em_andamento
