...
BEGIN
  ...
  IF NEW.valor_total IS NOT NULL AND NEW.valor_total > 0 THEN
    ...
    -- Taxa Cartão (now with fin_payables)
    ...
    -- Taxa Boleto (now with fin_payables)
    ...
    -- Taxa Link de Pagamento (now with fin_payables)
...
-- Backfill: create missing fin_payables for existing ledger entries (Taxas + RT)
INSERT INTO public.fin_payables (
  amount, due_date, competence_date, status, description,
  document_number, notes, ledger_entry_id, created_by,
  cost_center_id, project_id, order_id, chart_account_id
...
SELECT 
  le.amount,
  (le.competence_date + interval '30 days')::date,
  le.competence_date,
  'ABERTO',
  le.description,
  le.document_number,
...
JOIN public.orders o ON le.document_number = 'PED-' || o.order_number::text
WHERE le.type = 'DESPESA'
  AND le.parent_entry_id IS NOT NULL
  AND (le.description LIKE '%Taxa Cartão%' OR le.description LIKE '%Taxa Boleto%' OR le.description LIKE '%Taxa Link%' OR le.description LIKE '% RT (%')
  AND NOT EXISTS (
    SELECT 1 FROM public.fin_payables fp WHERE fp.ledger_entry_id = le.id
  );