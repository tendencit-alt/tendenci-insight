...
BEGIN
  ...
  IF NEW.taxa_link_valor IS NOT NULL AND NEW.taxa_link_valor > 0 AND NEW.taxa_link_responsavel = 'tendenci' THEN
    ...
    INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, document_number, party_id, party_type, status, notes, created_by, cost_center_id, project_id, parent_entry_id)
    VALUES ('Taxa Link Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_link_percentual::text,'0') || '%)', NEW.taxa_link_valor, 'DESPESA', v_competence_date, NULL, v_doc_number, v_fee_supplier_id, 'supplier', 'ABERTO', 'Taxa link', NEW.vendedor_id, v_cost_center_id, NULL, v_first_ledger_id) RETURNING id INTO v_expense_ledger_id;
    INSERT INTO public.fin_payables (supplier_id, amount, due_date, competence_date, description, document_number, ledger_entry_id, status, cost_center_id, notes)
    VALUES (v_fee_supplier_id, NEW.taxa_link_valor, v_first_due_date, v_competence_date, 'Taxa Link Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text), v_doc_number, v_expense_ledger_id, 'ABERTO', v_cost_center_id, 'Taxa de link');
  END IF;
...
SET
  ...
  project_id = COALESCE(fle.project_id, o.project_id),
  updated_at = now()
FROM public.orders o
WHERE fle.type = 'RECEITA'
  AND fle.document_number = 'PED-' || o.id::text
  AND (fle.tenant_id IS NULL OR fle.order_id IS NULL OR fle.client_id IS NULL OR fle.vendedor_id IS NULL);
...
JOIN public.orders o ON fle.document_number = 'PED-' || o.id::text
WHERE fr.ledger_entry_id = fle.id
  AND fle.type = 'RECEITA'
  AND (fr.tenant_id IS DISTINCT FROM o.tenant_id OR fr.order_id IS DISTINCT FROM o.id);