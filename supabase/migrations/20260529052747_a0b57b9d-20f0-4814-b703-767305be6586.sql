...
BEGIN
  ...
  IF NEW.taxa_cartao_valor IS NOT NULL AND NEW.taxa_cartao_valor > 0 AND NEW.taxa_cartao_responsavel = 'tendenci' THEN
    SELECT supplier_id, chart_account_id INTO v_fee_supplier_id, v_fee_chart_account_id FROM public.fee_supplier_configs WHERE fee_type = 'cartao' LIMIT 1;
    ...
    VALUES (v_fee_supplier_id, NEW.taxa_cartao_valor, v_first_due_date, v_competence_date, 'Taxa Cartão Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text), v_doc_number || '/TX-CARTAO', v_expense_ledger_id, 'ABERTO', v_cost_center_id, v_fee_chart_account_id, 'Taxa de cartão', NEW.tenant_id, NEW.id);
  END IF;

  IF NEW.taxa_boleto_valor IS NOT NULL AND NEW.taxa_boleto_valor > 0 AND NEW.taxa_boleto_responsavel = 'tendenci' THEN
    SELECT supplier_id, chart_account_id INTO v_fee_supplier_id, v_fee_chart_account_id FROM public.fee_supplier_configs WHERE fee_type = 'boleto' LIMIT 1;
    INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, document_number, party_id, party_type, status, notes, created_by, cost_center_id, project_id, parent_entry_id, chart_account_id, tenant_id, order_id)
    VALUES ('Taxa Boleto Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_boleto_percentual::text,'0') || '%)', NEW.taxa_boleto_valor, 'DESPESA', v_competence_date, NULL, v_doc_number || '/TX-BOLETO', v_fee_supplier_id, 'supplier', 'ABERTO', 'Taxa boleto', NEW.vendedor_id, v_cost_center_id, NULL, v_first_ledger_id, v_fee_chart_account_id, NEW.tenant_id, NEW.id) RETURNING id INTO v_expense_ledger_id;
    INSERT INTO public.fin_payables (supplier_id, amount, due_date, competence_date, description, document_number, ledger_entry_id, status, cost_center_id, chart_account_id, notes, tenant_id, order_id)
    VALUES (v_fee_supplier_id, NEW.taxa_boleto_valor, v_first_due_date, v_competence_date, 'Taxa Boleto Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text), v_doc_number || '/TX-BOLETO', v_expense_ledger_id, 'ABERTO', v_cost_center_id, v_fee_chart_account_id, 'Taxa de boleto', NEW.tenant_id, NEW.id);
  END IF;

  IF NEW.taxa_link_valor IS NOT NULL AND NEW.taxa_link_valor > 0 AND NEW.taxa_link_responsavel = 'tendenci' THEN
    SELECT supplier_id, chart_account_id INTO v_fee_supplier_id, v_fee_chart_account_id FROM public.fee_supplier_configs WHERE fee_type = 'link' LIMIT 1;
    INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, document_number, party_id, party_type, status, notes, created_by, cost_center_id, project_id, parent_entry_id, chart_account_id, tenant_id, order_id)
    VALUES ('Taxa Link Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_link_percentual::text,'0') || '%)', NEW.taxa_link_valor, 'DESPESA', v_competence_date, NULL, v_doc_number || '/TX-LINK', v_fee_supplier_id, 'supplier', 'ABERTO', 'Taxa link', NEW.vendedor_id, v_cost_center_id, NULL, v_first_ledger_id, v_fee_chart_account_id, NEW.tenant_id, NEW.id) RETURNING id INTO v_expense_ledger_id;
    INSERT INTO public.fin_payables (supplier_id, amount, due_date, competence_date, description, document_number, ledger_entry_id, status, cost_center_id, chart_account_id, notes, tenant_id, order_id)
    VALUES (v_fee_supplier_id, NEW.taxa_link_valor, v_first_due_date, v_competence_date, 'Taxa Link Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text), v_doc_number || '/TX-LINK', v_expense_ledger_id, 'ABERTO', v_cost_center_id, v_fee_chart_account_id, 'Taxa de link', NEW.tenant_id, NEW.id);
  END IF;
