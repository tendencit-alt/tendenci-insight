...
SET supplier_id = orr.supplier_id,
    description = 'PED #' || o.order_number || ' - ' || COALESCE(fca.name, 'Compromisso'),
    amount = osc.valor,
    chart_account_id = osc.chart_account_id,
    project_id = o.project_id,
    due_date = COALESCE(o.data_emissao::date, NOW()::date) + INTERVAL '30 days',
    competence_date = COALESCE(o.data_emissao::date, NOW()::date),
    updated_at = now()
FROM public.order_strategic_commitments osc
JOIN public.orders o ON o.id = osc.order_id
LEFT JOIN public.order_responsibles orr ON orr.id = osc.responsavel_id
LEFT JOIN public.fin_chart_accounts fca ON fca.id = osc.chart_account_id
WHERE fp.document_number = 'COMP-' || osc.id::text
  AND fp.origem = 'order_strategic_commitment'
  AND COALESCE(fp.status, 'ABERTO') IN ('ABERTO','VENCIDO')
  AND osc.habilitado = true
  AND osc.responsavel_id IS NOT NULL;

DELETE FROM public.fin_payables fp
USING public.order_strategic_commitments osc
WHERE fp.document_number = 'COMP-' || osc.id::text
  AND fp.origem = 'order_strategic_commitment'
  AND COALESCE(fp.status, 'ABERTO') IN ('ABERTO','VENCIDO')
  AND (
    COALESCE(osc.habilitado, false) = false
    OR COALESCE(osc.valor, 0) <= 0
    OR osc.responsavel_id IS NULL
  );