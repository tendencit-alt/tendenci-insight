...
BEGIN
  ...
  IF NEW.valor_total IS NOT NULL AND NEW.valor_total > 0 THEN
    ...
    IF COALESCE(NEW.rt_habilitado, false) = true AND COALESCE(NEW.rt_valor, 0) > 0 THEN
      ...
      -- Auto-create or find supplier from architect
      ...
      IF NEW.architect_id IS NOT NULL THEN
        ...
        IF v_arch_name IS NOT NULL THEN
          -- Try to find existing supplier by name match
          SELECT id INTO v_rt_supplier_id
            FROM public.suppliers
            WHERE name = v_arch_name
            LIMIT 1;

          -- If not found, create supplier from architect data
          IF v_rt_supplier_id IS NULL THEN
            ...
            VALUES (
              ...
              v_arch_city,
              v_arch_company,
              'Criado automaticamente a partir do arquiteto (RT)',
              true
            )
