CREATE OR REPLACE FUNCTION public.get_production_timeline(_tenant_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
AS $function$
...
BEGIN
  ...
  ops AS (
    SELECT po.*, c.name as client_name, ord.data_emissao, ord.updated_at as order_updated_at
      FROM public.production_orders po
      LEFT JOIN public.clients c ON c.id = po.client_id
      LEFT JOIN public.orders ord ON ord.id = po.order_id
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      -- STANDARD: return raw stored dates, no data_emissao override.
      ...
      -- Order emission date kept separate for reference.
      'order_emission_date', o.data_emissao,
