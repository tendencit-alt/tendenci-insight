...
BEGIN
  ...
  IF NEW.custo_total IS NOT NULL AND NEW.custo_total > 0 THEN
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Custo Produção Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' [CC: ' || v_cc_display_name || ']',
    ...
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Custo Produção Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' - ' || COALESCE(v_client_name, ''),
      ...
      INSERT INTO public.fin_payables (
        ...
        'Custo Produção Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text),
  ...
  IF NEW.taxa_cartao_valor IS NOT NULL AND NEW.taxa_cartao_valor > 0 AND NEW.taxa_cartao_responsavel = 'tendenci' THEN
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Taxa Cartão Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_cartao_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
    ...
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Taxa Cartão Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_cartao_percentual::text, '0') || '%)',
  ...
  IF NEW.taxa_boleto_valor IS NOT NULL AND NEW.taxa_boleto_valor > 0 AND NEW.taxa_boleto_responsavel = 'tendenci' THEN
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Taxa Boleto Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_boleto_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
          v_doc_number, v_fee_supplier_id, 'supplier', 'ABERTO',
    ...
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Taxa Boleto Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_boleto_percentual::text, '0') || '%)',
  ...
  IF NEW.taxa_link_valor IS NOT NULL AND NEW.taxa_link_valor > 0 AND NEW.taxa_link_responsavel = 'tendenci' THEN
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Taxa Link Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_link_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
          v_doc_number, v_fee_supplier_id, 'supplier', 'ABERTO',
    ...
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Taxa Link Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_link_percentual::text, '0') || '%)',
  ...
  -- 6. RT (Recurso Tendenci)
  IF NEW.rt_valor IS NOT NULL AND NEW.rt_valor > 0 THEN
    SELECT id INTO v_rt_supplier_id FROM public.fee_supplier_configs WHERE fee_type = 'rt' LIMIT 1;
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'RT Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.rt_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
          v_doc_number, v_rt_supplier_id, 'supplier', 'ABERTO',
          'Recurso Tendenci | CC: ' || v_cc_display_name,
          NEW.vendedor_id,
          v_cc_group.cc_id, NULL, v_first_ledger_id, v_strategic_chart_account_id
        ) RETURNING id INTO v_expense_ledger_id;
        ...
        INSERT INTO public.fin_payables (
          ...
          v_doc_number, v_expense_ledger_id, 'ABERTO', v_cc_group.cc_id,
          'Recurso Tendenci', v_strategic_chart_account_id
        );
      END LOOP;
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'RT Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.rt_percentual::text, '0') || '%)',
        NEW.rt_valor, 'DESPESA', v_competence_date, NULL,
        v_doc_number, v_rt_supplier_id, 'supplier', 'ABERTO',
        'Recurso Tendenci',
        NEW.vendedor_id,
        v_cost_center_id, NULL, v_first_ledger_id, v_strategic_chart_account_id
      ) RETURNING id INTO v_expense_ledger_id;
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Comissão Vendedor Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_vendedor_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
        ...
        INSERT INTO public.fin_payables (
          ...
          v_doc_number, v_expense_ledger_id, 'ABERTO', v_cc_group.cc_id,
          'Comissão do vendedor', v_strategic_chart_account_id
        );
      END LOOP;
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Comissão Vendedor Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_vendedor_percentual::text, '0') || '%)',
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Comissão Orçamentista Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_orcamentista_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
          v_doc_number, v_supplier_id, 'supplier', 'ABERTO',
        ...
        INSERT INTO public.fin_payables (
          ...
          v_doc_number, v_expense_ledger_id, 'ABERTO', v_cc_group.cc_id,
          'Comissão do orçamentista', v_strategic_chart_account_id
        );
      END LOOP;
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Comissão Orçamentista Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_orcamentista_percentual::text, '0') || '%)',
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Comissão Projetista Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_projetista_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
          v_doc_number, v_supplier_id, 'supplier', 'ABERTO',
        ...
        INSERT INTO public.fin_payables (
          ...
          v_doc_number, v_expense_ledger_id, 'ABERTO', v_cc_group.cc_id,
          'Comissão do projetista', v_strategic_chart_account_id
        );
      END LOOP;
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Comissão Projetista Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_projetista_percentual::text, '0') || '%)',
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Comissão Montador Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_montador_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
          v_doc_number, v_supplier_id, 'supplier', 'ABERTO',
        ...
        INSERT INTO public.fin_payables (
          ...
          v_doc_number, v_expense_ledger_id, 'ABERTO', v_cc_group.cc_id,
          'Comissão do montador', v_strategic_chart_account_id
        );
      END LOOP;
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Comissão Montador Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_montador_percentual::text, '0') || '%)',
    ...
    IF v_has_cc_groups THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_ledger_entries (
          ...
          'Comissão Produção Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_producao_percentual::text, '0') || '%) [CC: ' || v_cc_display_name || ']',
          v_proportional_amount, 'DESPESA', v_competence_date, NULL,
          v_doc_number, v_supplier_id, 'supplier', 'ABERTO',
        ...
        INSERT INTO public.fin_payables (
          ...
          v_doc_number, v_expense_ledger_id, 'ABERTO', v_cc_group.cc_id,
          'Comissão de produção', v_strategic_chart_account_id
        );
      END LOOP;
    ELSE
      INSERT INTO public.fin_ledger_entries (
        ...
        'Comissão Produção Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.comissao_producao_percentual::text, '0') || '%)',
  ...
  -- 12. Architect Commission
  IF NEW.architect_id IS NOT NULL AND NEW.comissao_arquiteto_valor IS NOT NULL AND NEW.comissao_arquiteto_valor > 0 THEN
    SELECT name, email, phone, city, company
    ...
    INSERT INTO public.fin_ledger_entries (
      ...
      'Comissão Arquiteto Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' - ' || COALESCE(v_arch_name, ''),
      NEW.comissao_arquiteto_valor, 'DESPESA', v_competence_date, NULL,
      v_doc_number, NEW.architect_id, 'architect', 'ABERTO',
      'Arq: ' || COALESCE(v_arch_name, '') || ' | Email: ' || COALESCE(v_arch_email, '') || ' | Tel: ' || COALESCE(v_arch_phone, '') || ' | Cidade: ' || COALESCE(v_arch_city, '') || ' | Empresa: ' || COALESCE(v_arch_company, ''),
      NEW.vendedor_id,
      v_cost_center_id, NULL, v_first_ledger_id
    ) RETURNING id INTO v_expense_ledger_id;
    ...
    INSERT INTO public.fin_payables (
      ...
      NULL, NEW.comissao_arquiteto_valor, v_competence_date, v_competence_date,
      'Comissão Arquiteto Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' - ' || COALESCE(v_arch_name, ''),
      v_doc_number, v_expense_ledger_id, 'ABERTO', v_cost_center_id,
      'Arq: ' || COALESCE(v_arch_name, '') || ' | ' || COALESCE(v_arch_email, '') || ' | ' || COALESCE(v_arch_phone, '')
    );
  END IF;
