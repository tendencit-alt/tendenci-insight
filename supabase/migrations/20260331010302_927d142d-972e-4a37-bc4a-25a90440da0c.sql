...
-- Step 1: Backfill cash_date for existing entries
UPDATE public.fin_ledger_entries
SET cash_date = competence_date
WHERE cash_date IS NULL;

-- Step 2: Update create_order_commission_entries to include cash_date
...
BEGIN
  IF (
    ...
    IF NEW.taxa_cartao_valor IS NOT NULL AND NEW.taxa_cartao_valor > 0 THEN
      ...
      INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, status, cost_center_id, project_id, chart_account_id)
      VALUES ('PED #' || NEW.order_number || ' - Taxa Cartão (' || ROUND(NEW.taxa_cartao_valor / NULLIF(NEW.valor_total,0) * 100, 2) || '%)',
        NEW.taxa_cartao_valor, 'DESPESA', COALESCE(NEW.data_emissao::date, NOW()::date), COALESCE(NEW.data_emissao::date, NOW()::date), 'ABERTO', v_cost_center_id, NEW.project_id, v_chart_account_id);
    END IF;
    ...
    IF NEW.taxa_boleto_valor IS NOT NULL AND NEW.taxa_boleto_valor > 0 THEN
      ...
      VALUES ('PED #' || NEW.order_number || ' - Taxa Boleto',
        NEW.taxa_boleto_valor, 'DESPESA', COALESCE(NEW.data_emissao::date, NOW()::date), COALESCE(NEW.data_emissao::date, NOW()::date), 'ABERTO', v_cost_center_id, NEW.project_id, v_chart_account_id);
    END IF;
    ...
    IF NEW.rt_valor IS NOT NULL AND NEW.rt_valor > 0 THEN
      ...
      VALUES ('PED #' || NEW.order_number || ' - RT (' || ROUND(NEW.rt_valor / NULLIF(NEW.valor_total,0) * 100, 2) || '%)',
        NEW.rt_valor, 'DESPESA', COALESCE(NEW.data_emissao::date, NOW()::date), COALESCE(NEW.data_emissao::date, NOW()::date), 'ABERTO', v_cost_center_id, NEW.project_id, v_chart_account_id);
    END IF;

    -- Comissões (vendedor, orçamentista, projetista, montador)
    IF NEW.comissao_vendedor_valor IS NOT NULL AND NEW.comissao_vendedor_valor > 0 THEN
      ...
      INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, status, cost_center_id, project_id, chart_account_id)
      VALUES ('PED #' || NEW.order_number || ' - Comissão Vendedor',
        NEW.comissao_vendedor_valor, 'DESPESA', COALESCE(NEW.data_emissao::date, NOW()::date), COALESCE(NEW.data_emissao::date, NOW()::date), 'ABERTO', v_cost_center_id, NEW.project_id, v_chart_account_id);
    END IF;

    IF NEW.comissao_orcamentista_valor IS NOT NULL AND NEW.comissao_orcamentista_valor > 0 THEN
      SELECT id INTO v_chart_account_id FROM public.fin_chart_accounts WHERE code = '3.1.2' LIMIT 1;
      INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, status, cost_center_id, project_id, chart_account_id)
      VALUES ('PED #' || NEW.order_number || ' - Comissão Orçamentista',
        NEW.comissao_orcamentista_valor, 'DESPESA', COALESCE(NEW.data_emissao::date, NOW()::date), COALESCE(NEW.data_emissao::date, NOW()::date), 'ABERTO', v_cost_center_id, NEW.project_id, v_chart_account_id);
    END IF;

    IF NEW.comissao_projetista_valor IS NOT NULL AND NEW.comissao_projetista_valor > 0 THEN
      SELECT id INTO v_chart_account_id FROM public.fin_chart_accounts WHERE code = '3.1.2' LIMIT 1;
      INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, status, cost_center_id, project_id, chart_account_id)
      VALUES ('PED #' || NEW.order_number || ' - Comissão Projetista',
        NEW.comissao_projetista_valor, 'DESPESA', COALESCE(NEW.data_emissao::date, NOW()::date), COALESCE(NEW.data_emissao::date, NOW()::date), 'ABERTO', v_cost_center_id, NEW.project_id, v_chart_account_id);
    END IF;

    IF NEW.comissao_montador_valor IS NOT NULL AND NEW.comissao_montador_valor > 0 THEN
      SELECT id INTO v_chart_account_id FROM public.fin_chart_accounts WHERE code = '3.1.2' LIMIT 1;
      INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, status, cost_center_id, project_id, chart_account_id)
      VALUES ('PED #' || NEW.order_number || ' - Comissão Montador',
        NEW.comissao_montador_valor, 'DESPESA', COALESCE(NEW.data_emissao::date, NOW()::date), COALESCE(NEW.data_emissao::date, NOW()::date), 'ABERTO', v_cost_center_id, NEW.project_id, v_chart_account_id);
    END IF;
  END IF;
  RETURN NEW;
END;
...
-- Step 3: Update create_receivable_from_order to set cash_date = competence_date
