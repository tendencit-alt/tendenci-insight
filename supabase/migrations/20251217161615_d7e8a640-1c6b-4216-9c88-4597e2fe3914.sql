-- Drop ALL versions of the function by specifying complete signatures
...
-- Recreate the correct version with unique project counting
...
BEGIN
  WITH stage_entries AS (
    SELECT 
      ph.project_id,
      CASE 
        WHEN ph.action_type = 'created' THEN 'recebido'
        WHEN ph.action_type = 'stage_change' THEN 
          LOWER(REPLACE(ph.new_value, ' ', '_'))
        ELSE NULL
      END as target_stage,
    ...
    WHERE 
      (ph.action_type = 'created' OR (ph.action_type = 'stage_change' AND ph.field_name = 'stage'))
