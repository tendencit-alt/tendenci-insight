...
-- Update trigger to propagate supplier_id to fin_payables
...
BEGIN
  ...
  IF NEW.valor_total IS NOT NULL AND NEW.valor_total > 0 THEN
    ...
    IF COALESCE(NEW.comissao_orcamentista_valor, 0) > 0 THEN
      ...
      END IF;
      SELECT COALESCE(src.chart_account_id, (SELECT id FROM public.fin_chart_accounts WHERE code = '3.1.3' LIMIT 1))
        INTO v_chart_account_id
    ...
    IF COALESCE(NEW.comissao_projetista_valor, 0) > 0 THEN
      ...
      END IF;
      SELECT COALESCE(src.chart_account_id, (SELECT id FROM public.fin_chart_accounts WHERE code = '3.1.4' LIMIT 1))
        INTO v_chart_account_id
    ...
    IF COALESCE(NEW.comissao_montador_valor, 0) > 0 THEN
      ...
      END IF;
      SELECT COALESCE(src.chart_account_id, (SELECT id FROM public.fin_chart_accounts WHERE code = '3.1.5' LIMIT 1))
        INTO v_chart_account_id
