...
BEGIN
  ...
  op_flags AS (
    SELECT
      ...
      -- atraso projetado: prazo NÃO venceu, mas ETA vai estourar
      (e.planned_end_date IS NOT NULL
         AND e.planned_end_date >= now()
      ...
      -- alerta de prazo: ETA dentro de 10% do prazo (zona amarela), ainda no prazo
      (e.planned_end_date IS NOT NULL
         AND e.planned_end_date >= now()
         AND e.eta <= e.planned_end_date
