...
BEGIN
  ...
  v_centro_custo_name := NULL;
  SELECT oi.centro_custo
    INTO v_centro_custo_name
    FROM public.order_items oi
   WHERE oi.order_id = NEW.id
     AND oi.centro_custo IS NOT NULL
     AND trim(oi.centro_custo) <> ''
   LIMIT 1;

  IF v_centro_custo_name IS NULL THEN
    v_centro_custo_name := NEW.centro_custo;
  END IF;

  v_cost_center_id := NULL;
  IF v_centro_custo_name IS NOT NULL AND length(trim(v_centro_custo_name)) > 0 THEN
    SELECT id INTO v_cost_center_id
    FROM public.fin_cost_centers
    WHERE tenant_id = NEW.tenant_id
      AND active = true
      AND (
        lower(name) = lower(trim(v_centro_custo_name))
        OR lower(name) = lower(
          CASE trim(v_centro_custo_name)
            WHEN 'moveis_planejados' THEN 'Planejados'
            WHEN 'producao_tendenci' THEN 'Produção Tendenci'
            WHEN 'revenda' THEN 'Revenda'
            ELSE trim(v_centro_custo_name)
          END
        )
      )
    LIMIT 1;
  ...
  IF NEW.taxa_cartao_valor IS NOT NULL AND NEW.taxa_cartao_valor > 0 AND NEW.taxa_cartao_responsavel = 'tendenci' THEN
    ...
    IF v_fee_supplier_id IS NULL AND v_fee_chart_account_id IS NULL THEN
      SELECT supplier_id, chart_account_id, cost_center_id
        ...
        LIMIT 1;
    END IF;
    v_fee_cc_final := v_cost_center_id;
    ...
    VALUES ('Taxa Cartão Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_cartao_percentual::text,'0') || '%)', NEW.taxa_cartao_valor, 'DESPESA', v_competence_date, NULL, v_doc_number || '/TX-CARTAO', v_fee_supplier_id, 'supplier', 'ABERTO', 'Taxa cartão', NEW.vendedor_id, v_fee_cc_final, NEW.project_id, v_first_ledger_id, v_fee_chart_account_id, NEW.tenant_id, NEW.id) RETURNING id INTO v_expense_ledger_id;
    INSERT INTO public.fin_payables (supplier_id, amount, due_date, competence_date, description, document_number, ledger_entry_id, status, cost_center_id, project_id, chart_account_id, notes, tenant_id, order_id)
    VALUES (v_fee_supplier_id, NEW.taxa_cartao_valor, v_first_due_date, v_competence_date, 'Taxa Cartão Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text), v_doc_number || '/TX-CARTAO', v_expense_ledger_id, 'ABERTO', v_fee_cc_final, NEW.project_id, v_fee_chart_account_id, 'Taxa de cartão', NEW.tenant_id, NEW.id);
  END IF;
  ...
  IF NEW.taxa_boleto_valor IS NOT NULL AND NEW.taxa_boleto_valor > 0 AND NEW.taxa_boleto_responsavel = 'tendenci' THEN
    ...
    INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, document_number, party_id, party_type, status, notes, created_by, cost_center_id, project_id, parent_entry_id, chart_account_id, tenant_id, order_id)
    VALUES ('Taxa Boleto Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_boleto_percentual::text,'0') || '%)', NEW.taxa_boleto_valor, 'DESPESA', v_competence_date, NULL, v_doc_number || '/TX-BOLETO', v_fee_supplier_id, 'supplier', 'ABERTO', 'Taxa boleto', NEW.vendedor_id, v_fee_cc_final, NEW.project_id, v_first_ledger_id, v_fee_chart_account_id, NEW.tenant_id, NEW.id) RETURNING id INTO v_expense_ledger_id;
    INSERT INTO public.fin_payables (supplier_id, amount, due_date, competence_date, description, document_number, ledger_entry_id, status, cost_center_id, project_id, chart_account_id, notes, tenant_id, order_id)
    VALUES (v_fee_supplier_id, NEW.taxa_boleto_valor, v_first_due_date, v_competence_date, 'Taxa Boleto Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text), v_doc_number || '/TX-BOLETO', v_expense_ledger_id, 'ABERTO', v_fee_cc_final, NEW.project_id, v_fee_chart_account_id, 'Taxa de boleto', NEW.tenant_id, NEW.id);
  END IF;
  ...
  IF NEW.taxa_link_valor IS NOT NULL AND NEW.taxa_link_valor > 0 AND NEW.taxa_link_responsavel = 'tendenci' THEN
    ...
    INSERT INTO public.fin_ledger_entries (description, amount, type, competence_date, cash_date, document_number, party_id, party_type, status, notes, created_by, cost_center_id, project_id, parent_entry_id, chart_account_id, tenant_id, order_id)
    VALUES ('Taxa Link Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' (' || COALESCE(NEW.taxa_link_percentual::text,'0') || '%)', NEW.taxa_link_valor, 'DESPESA', v_competence_date, NULL, v_doc_number || '/TX-LINK', v_fee_supplier_id, 'supplier', 'ABERTO', 'Taxa link', NEW.vendedor_id, v_fee_cc_final, NEW.project_id, v_first_ledger_id, v_fee_chart_account_id, NEW.tenant_id, NEW.id) RETURNING id INTO v_expense_ledger_id;
    INSERT INTO public.fin_payables (supplier_id, amount, due_date, competence_date, description, document_number, ledger_entry_id, status, cost_center_id, project_id, chart_account_id, notes, tenant_id, order_id)
    VALUES (v_fee_supplier_id, NEW.taxa_link_valor, v_first_due_date, v_competence_date, 'Taxa Link Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text), v_doc_number || '/TX-LINK', v_expense_ledger_id, 'ABERTO', v_fee_cc_final, NEW.project_id, v_fee_chart_account_id, 'Taxa de link', NEW.tenant_id, NEW.id);
  END IF;
