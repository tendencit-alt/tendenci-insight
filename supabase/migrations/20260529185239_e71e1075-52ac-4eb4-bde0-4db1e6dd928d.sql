...
DROP TABLE IF EXISTS public._e2e_results;
DROP TABLE IF EXISTS public._e2e_dbg;
DROP TABLE IF EXISTS public._e2e_dbg2;
DROP TABLE IF EXISTS public._e2e_dbg3;
DROP TABLE IF EXISTS public._e2e_dbg5;
...
BEGIN
  ...
  IF (v_dd = v_td) AND (v_di = v_ti) THEN
    v_target := 'entregue';
  ELSIF v_dd>0 OR v_di>0 THEN
    v_target := 'entrega_parcial';
  ELSE
    RETURN;
  END IF;

  -- Special: already at entrega_parcial and target is also entrega_parcial → no-op
  ...
  -- Walk through canonical path up to 'faturado'
  ...
  -- Final step: re-read order status and re-evaluate target (deliveries may have changed during nested triggers)
  SELECT status INTO v_prev FROM public.orders WHERE id=_order_id;
  IF v_prev IN ('entregue','encerrado','cancelado') THEN RETURN; END IF;

  -- Recompute target with freshest delivery state (defensive against nested trigger recursion)
  ...
  ELSIF v_target='entrega_parcial' AND v_prev='faturado' THEN
    UPDATE public.orders
     ...
     WHERE id=_order_id AND tenant_id=v_tenant AND status='faturado';
  ELSE
    RETURN;
  END IF;
  ...
  IF FOUND THEN
    ...
    VALUES(v_tenant,'orders',_order_id::text,'auto_promote','fulfillment_evaluate_order_status','status',v_prev,v_target,
           jsonb_build_object('origin','delivery_completion','order_id',_order_id,'previous_status',v_prev,'new_status',v_target,
                              'total_deliveries',v_td,'done_deliveries',v_dd,
                              'total_installations',v_ti,'done_installations',v_di));
    INSERT INTO public.cross_module_events
      (tenant_id,event_type,source_module,target_module,source_entity,source_entity_id,target_entity,target_entity_id,payload,status)
    VALUES(v_tenant,
