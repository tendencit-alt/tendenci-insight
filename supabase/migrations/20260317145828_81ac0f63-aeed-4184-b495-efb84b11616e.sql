...
-- Update both triggers: use order competence_date for all payable due_dates instead of NOW()+30 days
...
BEGIN
  IF (TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status 
    ...
    v_due_date := v_competence_date; -- All expenses use the order date
    ...
    IF NEW.valor_total IS NOT NULL AND NEW.valor_total > 0 THEN
      ...
      LOOP
        ...
        INSERT INTO public.fin_receivables (
          ...
          v_doc_number, 'Gerado automaticamente a partir do Pedido #' || NEW.order_number,
        ...
        IF COALESCE(NEW.comissao_vendedor_valor, 0) > 0 THEN
          ...
          INSERT INTO public.fin_payables (
            ...
            ROUND((NEW.comissao_vendedor_valor * v_item_proportion)::numeric, 2),
            v_due_date, v_competence_date, 'ABERTO',
