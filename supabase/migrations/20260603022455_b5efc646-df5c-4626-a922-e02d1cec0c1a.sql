-- 1. Prazo planejado por fase (na própria coluna do Kanban)
ALTER TABLE public.production_status_columns
  ADD COLUMN IF NOT EXISTS duration_days int;

-- Backfill: usa sla_days se existir, senão 7 (conservador)
UPDATE public.production_status_columns
   SET duration_days = COALESCE(NULLIF(sla_days, 0), 7)
 WHERE duration_days IS NULL;

-- 2. Override por OP × fase
CREATE TABLE IF NOT EXISTS public.production_order_phase_plan (
  ...
  UNIQUE (production_order_id, phase_slug)
);

CREATE INDEX IF NOT EXISTS idx_popp_op ON public.production_order_phase_plan(production_order_id);
CREATE INDEX IF NOT EXISTS idx_popp_tenant ON public.production_order_phase_plan(tenant_id);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.production_order_phase_plan TO authenticated;
GRANT ALL ON public.production_order_phase_plan TO service_role;

ALTER TABLE public.production_order_phase_plan ENABLE ROW LEVEL SECURITY;

CREATE POLICY "popp_select_tenant" ON public.production_order_phase_plan
  FOR SELECT TO authenticated
  USING (tenant_id = get_user_tenant_id() OR is_owner());
CREATE POLICY "popp_insert_tenant" ON public.production_order_phase_plan
  FOR INSERT TO authenticated
  WITH CHECK (tenant_id = get_user_tenant_id() OR is_owner());
CREATE POLICY "popp_update_tenant" ON public.production_order_phase_plan
  FOR UPDATE TO authenticated
  USING (tenant_id = get_user_tenant_id() OR is_owner())
  WITH CHECK (tenant_id = get_user_tenant_id() OR is_owner());
CREATE POLICY "popp_delete_tenant" ON public.production_order_phase_plan
  FOR DELETE TO authenticated
  USING (tenant_id = get_user_tenant_id() OR is_owner());

-- 3. RPC: timeline consolidado por tenant
...
BEGIN
  ...
  op_current AS (
    SELECT
      ...
      (SELECT MAX(sort_order) FROM phases p WHERE p.tenant_id = o.tenant_id) AS last_sort
    FROM ops o
  ),
  op_remaining AS (
    SELECT
      c.op_id,
      COALESCE(SUM(opc.duration_days) FILTER (WHERE opc.sort_order > c.current_sort), 0) AS remaining_days
    FROM op_current c
    ...
    SELECT opc.op_id,
           jsonb_agg(
             jsonb_build_object(
               ...
               'label', opc.label,
               'color', opc.color,
               'sort_order', opc.sort_order,
               'duration_days', opc.duration_days,
               'planned_start', opc.planned_start,
               'planned_end', opc.planned_end
  ...
  op_history AS (
    ...
    WHERE h.tenant_id = _eff_tenant OR _eff_tenant IS NULL
...
-- 4. Realtime para reflexão imediata entre abas (idempotente)
DO $$ BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE public.production_orders;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE public.production_order_phase_history;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE public.production_status_columns;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE public.production_order_phase_plan;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;