-- RPC para comparativo de vendedores em campanhas
...
BEGIN
  ...
  WITH campaign_stats AS (
    ...
    FROM tendenci_campaigns c
    LEFT JOIN tendenci_campaign_dispatches cd ON cd.campanha_id = c.id
  ...
  architect_response_stats AS (
    SELECT 
      ...
      END) as convertidos
    FROM tendenci_campaigns c
    JOIN tendenci_campaign_architects ca ON ca.campanha_id = c.id
    JOIN architects a ON a.id = ca.arquiteto_id
    WHERE c.created_at >= p_date_from 
      AND c.created_at <= p_date_to
      AND c.vendedor_id IS NOT NULL
    GROUP BY c.vendedor_id
