...
BEGIN
  ...
  -- Resolve cost center from order's centro_custo (name), within tenant
  v_cost_center_id := NULL;
  IF NEW.centro_custo IS NOT NULL AND length(trim(NEW.centro_custo)) > 0 THEN
    SELECT id INTO v_cost_center_id
    FROM public.fin_cost_centers
    WHERE tenant_id = NEW.tenant_id
      AND active = true
      AND lower(name) = lower(trim(NEW.centro_custo))
    LIMIT 1;
  END IF;
  -- Fallback to 'Planejados' if the order did not specify a valid CC
