-- Drop and recreate the RPC with orcado_value and apresentado_value
...
BEGIN
    ...
    SELECT 
      ...
      CASE 
        WHEN ph.description LIKE 'Estágio alterado%' THEN
          LOWER(TRIM(SPLIT_PART(SPLIT_PART(ph.description, 'para "', 2), '"', 1)))
  ...
  stage_values AS (
    SELECT 
      se.target_stage,
      SUM(COALESCE(p.estimated_value, 0)) as total_val
    FROM stage_entries se
  ...
  SELECT 
    ...
    COALESCE((SELECT cnt FROM stage_counts WHERE target_stage = 'em orçamento'), 0),
    COALESCE((SELECT cnt FROM stage_counts WHERE target_stage = 'orçado'), 0),
    COALESCE((SELECT total_val FROM stage_values WHERE target_stage = 'orçado'), 0),
    ...
    COALESCE((SELECT cnt FROM stage_counts WHERE target_stage = 'em negociação'), 0)
  ...
  WHERE p.deadline IS NOT NULL
    ...
    AND p.status NOT IN ('aprovado', 'perdido')
    AND (p_architect_id IS NULL OR p_architect_id = '00000000-0000-0000-0000-000000000000'::uuid OR p.architect_id = p_architect_id)
    AND (p_architect_id != '00000000-0000-0000-0000-000000000000'::uuid OR p.architect_id IS NULL);
  ...
  SELECT COALESCE(SUM(p.estimated_value), 0) INTO v_total_value
  FROM projects p
  WHERE EXISTS (
