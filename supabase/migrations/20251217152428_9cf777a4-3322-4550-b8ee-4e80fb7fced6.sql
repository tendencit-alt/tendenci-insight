-- Fix architect filter logic in projects_metrics_by_history RPC
-- Bug: when p_architect_id IS NULL, the second condition "(NULL != '00000000...' OR p.architect_id IS NULL)" 
-- incorrectly filters to only projects WITHOUT an architect
...
BEGIN
  ...
  WITH stage_entries AS (
    ...
    WHERE 
      ...
      -- FIXED: Correct architect filter logic
      AND (
        p_architect_id IS NULL  -- NULL means show ALL projects
        OR (p_architect_id = '00000000-0000-0000-0000-000000000000'::uuid AND p.architect_id IS NULL)  -- Special UUID means only no-architect
        OR (p_architect_id != '00000000-0000-0000-0000-000000000000'::uuid AND p.architect_id = p_architect_id)  -- Valid UUID means that specific architect
      )
  ),
  stage_counts AS (
    SELECT 
