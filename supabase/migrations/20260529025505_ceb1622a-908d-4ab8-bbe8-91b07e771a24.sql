...
-- 1) Drop legacy duplicate-receivable trigger + function
DROP TRIGGER IF EXISTS trigger_create_receivable_from_order ON public.orders;
DROP FUNCTION IF EXISTS public.create_receivable_from_order();

-- 2) Rewrite update_financial_entries_on_order_edit so the receivable
--    respects data_primeiro_vencimento and splits into installments (parcelas)
...
BEGIN
  ...
  v_interval_days := 30; -- default monthly
  ...
  IF v_has_cc_groups THEN
    ...
    LOOP
      ...
      FOR v_i IN 1..v_installments LOOP
        ...
        VALUES (NEW.client_id, v_installment_amount, v_installment_due, v_competence_date,
          'Receita Pedido #' || COALESCE(NEW.order_number::text, NEW.id::text) || ' [CC: ' || v_cc_display_name || ']' ||
            CASE WHEN v_installments > 1 THEN ' (' || v_i || '/' || v_installments || ')' ELSE '' END,
...
-- 3) Cleanup existing duplicate receivable + its ledger for orders already created by the legacy trigger
DELETE FROM public.fin_receivables
WHERE document_number ~ '^PED-[0-9]+$'
  AND ledger_entry_id IN (
    SELECT id FROM public.fin_ledger_entries WHERE document_number ~ '^PED-[0-9]+$'
  );

DELETE FROM public.fin_ledger_entries
WHERE document_number ~ '^PED-[0-9]+$' AND type = 'RECEITA';

-- 4) Rebuild receivables for order #10 (and any other affected) using fixed function
-- Trigger by no-op touch
UPDATE public.orders SET updated_at = now() WHERE id IN (
  SELECT DISTINCT order_id FROM public.fin_receivables WHERE order_id IS NOT NULL
  UNION
  SELECT id FROM public.orders WHERE status IN ('ativo','faturado','em_producao')
) AND status IN ('ativo','faturado','em_producao');
