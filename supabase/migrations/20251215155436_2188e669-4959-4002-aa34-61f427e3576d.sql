-- Atualizar RPC get_campaign_metrics para:
-- 1. Incluir contato_iniciado + parceiro_ativo na taxa de resposta
-- 2. Adicionar filtro por vendedor

DROP FUNCTION IF EXISTS get_campaign_metrics(timestamp with time zone, timestamp with time zone);
...
BEGIN
  ...
  -- Respostas recebidas: contato_iniciado OU parceiro_ativo (não apenas contato_iniciado)
...
-- Atualizar get_campaign_evolution também para filtrar por vendedor
DROP FUNCTION IF EXISTS get_campaign_evolution(timestamp with time zone, timestamp with time zone);
