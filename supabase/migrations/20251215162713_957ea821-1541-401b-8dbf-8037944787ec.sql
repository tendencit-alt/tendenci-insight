-- Corrigir RPC para calcular taxas corretamente
...
BEGIN
  ...
  WITH campaign_stats AS (
    SELECT 
      ...
      COUNT(DISTINCT ca.architect_id) as total_arq
  ...
  SELECT 
    ...
    COALESCE(cs.total_arq, 0)::BIGINT as total_arquitetos,
    COALESCE(ars.respostas, 0)::BIGINT as respostas,
    COALESCE(ars.convertidos, 0)::BIGINT as convertidos,
    CASE 
      WHEN COALESCE(cs.total_arq, 0) > 0 
      THEN ROUND((COALESCE(ars.respostas, 0)::NUMERIC / cs.total_arq) * 100, 1)
      ELSE 0 
    END as taxa_resposta,
    CASE 
      WHEN COALESCE(cs.total_arq, 0) > 0 
      THEN ROUND((COALESCE(ars.convertidos, 0)::NUMERIC / cs.total_arq) * 100, 1)
      ELSE 0 
  ...
  ORDER BY COALESCE(cs.total_arq, 0) DESC;
END;
$$;