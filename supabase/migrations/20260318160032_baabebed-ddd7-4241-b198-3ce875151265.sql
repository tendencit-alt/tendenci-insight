-- Update financial automation for strategic resources:
-- 1) include project name in generated descriptions
-- 2) ensure project and cost center are auto-filled consistently
-- 3) include Produção in strategic resource generation
      ...
      LOOP
        ...
        IF COALESCE(NEW.rt_habilitado, false) = true AND COALESCE(NEW.rt_valor, 0) > 0 THEN
          INSERT INTO public.fin_ledger_entries (
            ...
            party_id, party_type, status, notes, created_by, cost_center_id, project_id, parent_entry_id
          ) VALUES (
        ...
        IF COALESCE(NEW.comissao_vendedor_valor, 0) > 0 THEN
          ...
          INSERT INTO public.fin_payables (
            ...
            notes, ledger_entry_id, created_by, cost_center_id, project_id, supplier_id
          ) VALUES (
            ROUND((NEW.comissao_vendedor_valor * v_item_proportion)::numeric, 2),
            v_due_date,
            v_competence_date,
...
BEGIN
  ...
  IF NEW.valor_total IS NOT DISTINCT FROM OLD.valor_total
    ...
    AND NEW.comissao_producao_percentual IS NOT DISTINCT FROM OLD.comissao_producao_percentual
    AND NEW.comissao_producao_responsavel_id IS NOT DISTINCT FROM OLD.comissao_producao_responsavel_id
    AND NEW.comissao_producao_responsible_id IS NOT DISTINCT FROM OLD.comissao_producao_responsible_id
