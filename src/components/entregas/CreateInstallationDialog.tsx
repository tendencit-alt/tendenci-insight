...
import { useCreateInstallation, useOrdersForFulfillment } from "@/hooks/useFulfillment";

export function CreateInstallationDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  async function submit() {
    ...
    await create.mutateAsync({
      ...
      equipe_responsavel: team || null,
      endereco: endereco || inferred || null,
      observacoes: obs || null,
    });
    onOpenChange(false);
    setOrderId(""); setScheduled(""); setTeam(""); setEndereco(""); setObs("");
  }

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Nova montagem</DialogTitle></DialogHeader>
        <div className="space-y-3">
          ...
          <div>
            ...
            <Input value={endereco || inferred} onChange={(e) => setEndereco(e.target.value)} />
          </div>
          <div>
