...
export function PerformanceKPIs({ kpis, arquitetosResumo }: PerformanceKPIsProps) {
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card className="hover:shadow-lg transition-shadow">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Parceiros Profissionais Engajados</CardTitle>
      ...
      <Card className="hover:shadow-lg transition-shadow">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Vendas via Parceiros Profissionais</CardTitle>
          <Briefcase className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold text-primary">{formatCurrency(arquitetosResumo.total_vendido_arquitetos)}</div>
          <p className="text-xs text-muted-foreground">Total com parceiros profissionais</p>
        </CardContent>
      </Card>
