...
import { Landmark, ArrowRightLeft, PiggyBank, RefreshCw } from "lucide-react";
...
export function TreasuryTab({ filters: _filters }: TreasuryTabProps) {
  ...
  const totalBalance = useMemo(() => {
    ...
    return bankAccounts.reduce((sum, acc) => sum + (acc.opening_balance || 0), 0);
  }, [bankAccounts]);
            ...
            <TableBody>
              {bankAccounts?.map((acc) => (
                <TableRow key={acc.id}>
                  ...
                  <TableCell className={`text-right font-mono font-medium ${(acc.opening_balance || 0) >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                    {formatCurrency(acc.opening_balance || 0)}
                  </TableCell>
                </TableRow>
              ))}
