...
export function FollowupDispatchPanel() {
    ...
    queryFn: async (): Promise<{ followup: CooldownInfo; group: CooldownInfo }> => {
      ...
      if (lastGroup?.[0]?.group_invite_sent_at) {
        ...
        if (minutesRemaining > 0) {
          ...
          groupCooldown.lastDispatch = { time: lastTime.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }), source: 'unknown', type: 'group' };
        }
      }
  ...
  const dispatchFollowups = async () => {
    ...
    try {
      ...
      const result: DispatchResult = {
        type: 'followup', success: true, dispatched: data?.dispatched || 0, failed: data?.failed || 0,
        message: data?.message || `${data?.dispatched || 0} follow-ups enviados`, timestamp: new Date(), source: 'manual'
      };
      ...
      setLastResults(prev => [{ type: 'followup', success: false, dispatched: 0, failed: 0, message: err.message, timestamp: new Date(), source: 'manual' }, ...prev].slice(0, 5));
  ...
  const dispatchGroupInvites = async () => {
    ...
    try {
      ...
      const result: DispatchResult = {
        type: 'group', success: true, dispatched: data?.dispatched || 0, failed: data?.failed || 0,
        message: data?.message || `${data?.dispatched || 0} convites enviados`, timestamp: new Date(), source: 'manual'
      };
      ...
      setLastResults(prev => [{ type: 'group', success: false, dispatched: 0, failed: 0, message: err.message, timestamp: new Date(), source: 'manual' }, ...prev].slice(0, 5));
      toast({ title: "Erro", description: err.message, variant: "destructive" });
      setShowMonitor(false);
    } finally {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 border rounded-lg space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-medium flex items-center gap-2">
                    <MessageSquare className="h-4 w-4 text-blue-500" />Follow-ups I.A.
                  </h4>
                  <p className="text-sm text-muted-foreground mt-1">Envia mensagens para deals sem resposta há 48h.</p>
                </div>
                <Badge variant="outline" className="text-xs flex-shrink-0"><Clock className="h-3 w-3 mr-1" />48h</Badge>
              </div>
              {cooldownInfo?.followup?.active && (
                <div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-500/10 px-2 py-1.5 rounded">
                  ...
                  <span>Último ({cooldownInfo.followup.lastDispatch?.source}) às {cooldownInfo.followup.lastDispatch?.time} • {cooldownInfo.followup.minutesRemaining}min restantes</span>
                </div>
              )}
              <Button onClick={dispatchFollowups} disabled={isDispatchingFollowup} className="w-full">
                {isDispatchingFollowup ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Enviando...</> : <><MessageSquare className="h-4 w-4 mr-2" />Disparar Follow-ups</>}
              </Button>
            </div>
            ...
            <div className="p-4 border rounded-lg space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-medium flex items-center gap-2">
                    <Users className="h-4 w-4 text-green-500" />Convites de Grupo
                  </h4>
                  <p className="text-sm text-muted-foreground mt-1">Convites para grupo WhatsApp após 7 dias.</p>
                </div>
                <Badge variant="outline" className="text-xs flex-shrink-0"><Clock className="h-3 w-3 mr-1" />7 dias</Badge>
              </div>
              {cooldownInfo?.group?.active && (
                <div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-500/10 px-2 py-1.5 rounded">
                  <Timer className="h-3 w-3" />
                  <span>Último às {cooldownInfo.group.lastDispatch?.time} • {cooldownInfo.group.minutesRemaining}min restantes</span>
                </div>
              )}
              <Button onClick={dispatchGroupInvites} disabled={isDispatchingGroup} variant="outline" className="w-full">
                {isDispatchingGroup ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Enviando...</> : <><Users className="h-4 w-4 mr-2" />Enviar Convites</>}
              </Button>
            </div>
          </div>
            ...
            <div className="border-t pt-4 space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {lastResults.map((result, index) => (
                  <div key={index} className={`flex items-center justify-between p-2 rounded text-sm ${result.success ? 'bg-green-500/10' : 'bg-destructive/10'}`}>
                    <div className="flex items-center gap-2">
                      {result.success ? <CheckCircle className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-destructive" />}
                      <span className="text-xs font-medium">{result.type === 'followup' ? 'Follow-up' : 'Grupo'}</span>
                      <span className="text-xs text-muted-foreground truncate max-w-[120px]">{result.message}</span>
                    </div>
                    <span className="text-xs text-muted-foreground flex-shrink-0">{formatTime(result.timestamp)}</span>
                  </div>
                ))}
              </div>
            </div>
